  ...
  return (
    <div className="glass overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 min-w-0 hover:shadow-lg transition-shadow duration-200">
      ...
      <div className="p-4 sm:p-6 border-t border-white/6 bg-slate-950/20">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`rounded-full px-2 py-1 text-sm ${vote === 'like' ? 'bg-green-600 text-white' : 'bg-white/3 text-brand-200'}`}
            >
            ...
            <button
              ...
              className={`rounded-full px-2 py-1 text-sm ${vote === 'dislike' ? 'bg-rose-600 text-white' : 'bg-white/3 text-slate-300'}`}
            >
        ...
        <p className="mt-3 text-sm text-slate-300">Un bref reportage présentant les moments forts, les interviews et les démonstrations. Idéal pour partager sur les réseaux ou l'intégrer dans le dossier de presse.</p>
        {/* dynamic page url/title and colored icons */}
        <div className="mt-3 flex items-center gap-3">
          {(() => {
            const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://baol.example.com'
            const pageTitle = typeof document !== 'undefined' ? document.title || 'Reportage Baol Digital Show' : 'Reportage Baol Digital Show'
            return (
              <>
                <a
