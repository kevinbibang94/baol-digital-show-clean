...
function Thumbnail({ src, alt, className, sizes }: { src: string; alt: string; className?: string; sizes?: string }) {
  ...
  // Derive generated filenames (generated by scripts/gen:images)
  ...
  return (
    <div className={`relative rounded-lg overflow-hidden bg-slate-800 ${className || ''} focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400`} style={{ minWidth: 0 }}>
      {/* low-quality placeholder layer (blurred) */}
      {placeholder && (
        <div
          aria-hidden
          className={`absolute inset-0 bg-cover bg-center transform scale-105 filter transition-opacity duration-500 ${loaded ? 'opacity-0' : 'opacity-100'}`}
          style={{ backgroundImage: `url(${placeholder})`, WebkitFilter: 'blur(8px)', filter: 'blur(8px)' }}
        />
      )}
      <picture>
    ...
    function onScroll() {
      rafId = requestAnimationFrame(() => {
        ...
        nodes.forEach((img) => {
          ...
          // compute ratio centered: -0.12..0.12
          const ratio = (rect.top + rect.height / 2 - windowH / 2) / (windowH / 2)
          const translate = Math.max(-1, Math.min(1, ratio)) * 12 // px
      ...
      <section className="border-b border-white/5 bg-slate-950/60 py-10 sm:py-12 lg:py-16">
        <div className="container space-y-6 sm:space-y-8">
          ...
          <div className="space-y-3">
            <p className="text-xs sm:text-sm font-semibold text-slate-300">Filtrer par catégorie :</p>
            ...
            <ol className="relative border-l border-white/10 pl-6 sm:pl-8">
              {filteredProgram.map((item, index) => (
                ...
                >
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4 group-hover:scale-102 transition-transform duration-200">
                      {item.image && (
                        <Thumbnail
                          src={item.image}
                          alt={item.title}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {item.tags.map((t: string) => (
                            <span key={t} className="rounded-full bg-white/5 px-3 py-1 text-xs font-medium text-slate-200">{t}</span>
                          ))}
                        </div>
                      </div>
                    </div>
          ...
          {/* Modal for details */}
          <Modal isOpen={!!selectedItem} onClose={() => setSelectedItem(null)} item={selectedItem} speakersData={speakers} reserveUrl={googleFormUrl} />
        </div>
      </section>
    </main>
  )
